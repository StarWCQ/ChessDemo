...
//    private static final Dimension CHESSGRID_SIZE = new Dimension(1080 / 4 * 3 / 8, 1080 / 4 * 3 / 8);
    ...
    private int size;
    public static int specialJudge[][] = new int[8][8];
    ...
    protected ChessComponent(ChessboardPoint chessboardPoint, Point location, ChessColor chessColor, ClickController clickController, int size) {
        ...
        gridColor = BACKGROUND_COLORS[(chessboardPoint.getX() + chessboardPoint.getY()) % 2];//给棋盘上色
        this.selected = false;
...
//        g.setColor(squareColor);
    ...
    public void updateColor(){
        if(canMove){
            gridColor = new Color(50,120,150);
        }
        else{
            gridColor = BACKGROUND_COLORS[(chessboardPoint.getX() + chessboardPoint.getY()) % 2];
        }
        repaint();//给棋盘上色
    }
    ...
    public void showCanMove(){
        for(int i = 0;i < 8;i++) {
            for (int j = 0; j < 8; j++) {
                if (canMoveTo(ChessGameFrame.chessboard.getChessComponents(), new ChessboardPoint(i, j)) && isSelected()) {
//                        g.drawOval(0,0,getWidth(),getHeight());
                    ChessGameFrame.chessboard.chessComponents[i][j].setCanMove(true);
                    ChessGameFrame.chessboard.chessComponents[i][j].updateColor();
                } else {
                    ChessGameFrame.chessboard.chessComponents[i][j].setCanMove(false);
                    ChessGameFrame.chessboard.chessComponents[i][j].updateColor();
                }
            }
        }
